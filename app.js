...
let data = { events: [], players: [], drivers: [], rides: [], driverProfiles: [], directory: [], locations: [] };
let selected = null;
  ...
  root.innerHTML = `
    ...
    <section class="workspace"><div class="sectionHeading"><div><p class="eyebrow">CARPOOL BOARD</p><h2>Games & practices</h2></div><div class="summaryPills"><span><b>${need}</b> need a seat</span><span><b>${seats}</b> seats offered</span></div></div>
      ...
      ${!data.events.length ? `<div class="empty"><div>⚽</div><h3>The schedule is coming soon</h3><p>An administrator will add the next game or practice. Once it appears, families can offer seats or request a ride.</p></div>` : `
        ...
        <div class="eventHeader"><div><p>${h(longDate(event.eventDate))} · Starts ${h(event.departTime)}</p><h3>${h(event.title)}</h3>${eventLocation?`<a class="mapLink" href="${h(eventLocation.mapUrl)}" target="_blank" rel="noreferrer">📍 ${h(event.location)}</a>`:`<span>${h(event.location)}</span>`}</div><div class="eventButtons"><button class="outline" data-open="driver">Offer seats</button><button class="primary" data-open="ride">Request ride</button></div></div>
        <div class="boardGrid">
          ...
          <section class="boardColumn"><div class="columnTitle"><div><span class="dot amber"></span><h3>Ride requests</h3></div><b>${rides.length}</b></div>
            ${rides.length ? rides.map(ride=>{
              const player=players.get(ride.playerId),pickup=locations.get(ride.locationId),options=drivers.filter(driver=>driver.id===ride.driverId||((driver.direction===ride.direction||driver.direction==="roundtrip")&&rides.filter(r=>r.driverId===driver.id).length<driver.capacity));
              return `<article class="card ${ride.driverId?"assigned":""}"><div class="cardTop"><div class="avatar player">${h(initials(player?.name||"P"))}</div><div><h4>${h(player?.name||"Player")}</h4><p>${h(direction(ride.direction))}</p></div><span class="badge ${ride.driverId?"":"waiting"}">${ride.driverId?"Matched":"Needs ride"}</span></div>${pickup?`<a class="mapLink rideLocation" href="${h(pickup.mapUrl)}" target="_blank" rel="noreferrer">📍 ${h(pickup.name)}</a>`:""}<label class="assign">Assigned driver<select data-assign="${ride.id}"><option value="">Unassigned</option>${options.map(driver=>`<option value="${driver.id}" ${driver.id===ride.driverId?"selected":""}>${h(driver.parentName)}</option>`).join("")}</select></label></article>`;
...
function openAdmin(){
  ...
  const modal=document.querySelector("#adminModal"),body=modal.querySelector("#adminBody");let password="",counts={schedule:0,players:0,drivers:0,locations:0},profiles=[],records={schedule:[],players:[],drivers:[],locations:[]},kind="schedule",view="manage",editing=null,confirming=false,notice="";
  ...
  const title=item=>kind==="schedule"?item.title:kind==="players"||kind==="locations"?item.name:kind==="drivers"?item.parentName:item.displayName;
  const meta=item=>kind==="schedule"?`${item.eventType} · ${item.eventDate} · ${item.departTime}`:kind==="players"?`${item.guardian} · ${item.phone}`:kind==="drivers"?`${item.capacity} seats · ${item.phone}`:kind==="locations"?"Google Maps destination":`${item.email} · ${item.approved?"Approved":"Pending"}`;
  const value=(item,key)=>h(item?.[key]??"");
  const editor=()=>{const item=editing||{};if(kind==="parents")return `<div class="parentReview"><dl><dt>Name</dt><dd>${value(item,"displayName")}</dd><dt>Email</dt><dd>${value(item,"email")}</dd><dt>Phone</dt><dd>${value(item,"phone")||"Not provided"}</dd><dt>Home pickup</dt><dd>${value(item,"homePickup")||"Not provided"}</dd><dt>Alternate pickup</dt><dd>${value(item,"alternatePickup")||"Not provided"}</dd><dt>Vehicle</dt><dd>${value(item,"vehicleType")||"Not provided"} · ${item.defaultSeats||3} seats</dd></dl><button class="${item.approved?"outline":"primary"}" data-approve="${item.id}" data-status="${item.approved?0:1}">${item.approved?"Pause parent access":"Approve parent"}</button></div>`;
  ...
  const showTools=()=>{const help=adminHelp[kind]||adminHelp.players;body.innerHTML=`<div class="adminCounts"><span><b>${counts.schedule}</b> events</span><span><b>${counts.players}</b> players</span><span><b>${counts.locations}</b> locations</span><span><b>${profiles.filter(item=>item.approved).length}</b> approved parents</span></div><div class="adminViewTabs"><button class="${view==="manage"?"active":""}" data-view="manage">Manage records</button><button class="${view==="upload"?"active":""}" data-view="upload">Bulk CSV upload</button></div>${view==="upload"?`<div class="importTabs">${Object.keys(adminHelp).map(item=>`<button class="${item===kind?"active":""}" data-kind="${item}">${adminHelp[item].title}</button>`).join("")}</div><form id="adminImport" class="importForm"><h3>Load ${help.title.toLowerCase()} from CSV</h3><p>First row must use these columns:</p><code>${h(help.columns)}</code><small>Example: ${h(help.sample)}</small><label>CSV file<input name="file" type="file" accept=".csv,text/csv" required></label><button class="primary submit">Load CSV</button></form>`:`<div class="importTabs">${[["schedule","Events"],["players","Players"],["drivers","Drivers"],["locations","Locations"],["parents","Parents"]].map(([key,label])=>`<button class="${key===kind?"active":""}" data-kind="${key}">${label}</button>`).join("")}</div>${editing?`<section class="recordEditor"><button class="backButton" data-back>← Back to list</button><h3>${editing.id?`Review ${h(title(editing))}`:`Add ${kind==="schedule"?"event":kind==="players"?"player":kind==="drivers"?"driver profile":"location"}`}</h3>${editor()}${editing.id&&editing.email!==currentUser?.email?`<div class="removeZone">${confirming?`<div><p>This permanently removes this ${kind==="parents"?"parent profile":"record"}${kind==="schedule"?" and its rides and drivers":""}.</p><div><button class="danger" data-remove>Yes, remove it</button><button class="outline" data-cancel>Cancel</button></div></div>`:`<button class="dangerLink" data-confirm>Remove this ${kind==="parents"?"parent profile":"record"}</button>`}</div>`:""}</section>`:`<section class="recordManager"><div class="recordManagerHead"><div><h3>${kind==="schedule"?"Schedule events":kind==="players"?"Player profiles":kind==="drivers"?"Driver profiles":kind==="locations"?"Team locations":"Parent accounts"}</h3><p>Select a record to review, edit, or remove it.</p></div>${kind!=="parents"?`<button class="primary small" data-add>Add new</button>`:""}</div>${list().length?`<div class="recordList">${list().map(item=>`<button data-record="${item.id}"><span><b>${h(title(item))}</b><small>${h(meta(item))}</small></span><strong>Review →</strong></button>`).join("")}</div>`:`<div class="inlineEmpty"><p>No records yet.</p></div>`}</section>`}`}${notice?`<div class="notice" role="status">${h(notice)}</div>`:""}`;
    ...
    body.querySelectorAll("[data-kind]").forEach(button=>button.addEventListener("click",()=>{kind=button.dataset.kind;editing=null;confirming=false;notice="";showTools()}));
    ...
    body.querySelectorAll("[data-record]").forEach(button=>button.addEventListener("click",()=>{editing=list().find(item=>String(item.id)===button.dataset.record);showTools()}));
...
function openModal(kind) {
  ...
  if(kind==="ride") fields=`<label>Game or practice<select name="eventId">${eventOptions()}</select></label>${data.players.length?`<label>Player<select name="playerId" required><option value="">Select a player</option>${data.players.map(p=>`<option value="${p.id}">${h(p.name)}</option>`).join("")}</select></label>`:`<div class="notice">Add a player first.</div><button type="button" class="outline" data-add-player>Add player</button>`}${data.locations?.length?`<label>Pickup / meeting location<select name="locationId" required><option value="">Choose a location</option>${data.locations.map(item=>`<option value="${item.id}">${h(item.name)}</option>`).join("")}</select></label>`:`<div class="notice">An administrator needs to add team locations before ride requests can be submitted.</div>`}${trip()}${field("notes","Notes","text","Booster seat, timing, etc.",false)}`;
  document.body.insertAdjacentHTML("beforeend",`<div class="modalBackdrop" id="modal"><div class="modal"><button class="close" aria-label="Close">×</button><p class="eyebrow">TEAM RIDE BOARD</p><h2>${titles[kind]}</h2><form id="modalForm">${fields}<button class="primary submit" ${(kind==="ride"&&(!data.players.length||!data.locations?.length))?"disabled":""}>${kind==="player"?"Save player":kind==="driver"?"Offer seats":"Request ride"}</button></form></div></div>`);
