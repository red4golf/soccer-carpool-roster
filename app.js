...
function openAdmin(){
  ...
  const editor=()=>{const item=editing||{};if(kind==="parents")return `<div class="parentReview"><dl><dt>Name</dt><dd>${value(item,"displayName")}</dd><dt>Email</dt><dd>${value(item,"email")}</dd><dt>Phone</dt><dd>${value(item,"phone")||"Not provided"}</dd><dt>Home pickup</dt><dd>${value(item,"homePickup")||"Not provided"}</dd><dt>Alternate pickup</dt><dd>${value(item,"alternatePickup")||"Not provided"}</dd><dt>Vehicle</dt><dd>${value(item,"vehicleType")||"Not provided"} · ${item.defaultSeats||3} seats</dd></dl><button class="${item.approved?"outline":"primary"}" data-approve="${item.id}" data-status="${item.approved?0:1}">${item.approved?"Pause parent access":"Approve parent"}</button></div>`;
    if(kind==="schedule")return `<form id="recordForm"><label>Event type<select name="eventType"><option value="practice" ${item.eventType!=="game"?"selected":""}>Practice</option><option value="game" ${item.eventType==="game"?"selected":""}>Game</option></select></label><label>Event title<input name="title" value="${value(item,"title")}" required></label><div class="two"><label>Date<input name="eventDate" type="date" value="${value(item,"eventDate")}" required></label><label>Departure time<input name="departTime" type="time" value="${value(item,"departTime")}" required></label></div><label>Destination / field<input name="location" value="${value(item,"location")}" required></label><label>Meet at<input name="meetAt" value="${value(item,"meetAt")}" required></label><label>Notes<input name="notes" value="${value(item,"notes")}"></label><button class="primary submit">Save record</button></form>`;
